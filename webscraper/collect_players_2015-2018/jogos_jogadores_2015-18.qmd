...
source(glue::glue(path,"/webscraper/functions/extract_info_matches_players_2015-18.R", sep=""))
```
...
lista_urls <- dbGetQuery(con,
                         "SELECT links_completos
                          ...
                          WHERE YEAR(data) BETWEEN 2015 AND 2018;"
                         )
```
...
Neste bloco, um loop é iniciado para percorrer cada URL na lista. Os dados são coletados usando a função processar_partida(). Para evitar problemas de limite de requisições, há uma pausa de 10 segundos entre as solicitações
```{r}
